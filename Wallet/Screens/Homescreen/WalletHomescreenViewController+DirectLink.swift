...
extension WalletHomescreenViewController {
    func addCertificateDirectly(url: URL, secret: String, secretSignature: String, clientId: String?, clientIdSignature: String?) {
        activeDirectLinkURL = url

        directLinkActionPopupView.addCertificateTouchUpCallback = { [weak self] birthday in
            self?.addCertificateTouchUpCallback(secret: secret, secretSignature: secretSignature, clientId: clientId, clientIdSignature: clientIdSignature, birthday: birthday)
        }
    ...
    func addCertificateTouchUpCallback(secret: String, secretSignature: String, clientId: String?, clientIdSignature: String?, birthday: Date?) {
        let request: URLRequest

        if let clientId = clientId, let clientIdSignature = clientIdSignature {
            request = getRequestWithBirthday(secret: secret, secretSignature: secretSignature, clientId: clientId, clientIdSignature: clientIdSignature, birthday: birthday)
        } else {
            request = getRequestWithBirthday(secret: secret, secretSignature: secretSignature, birthday: birthday)
        }

        HttpRequestHandler.sendRequest(request, handleResponse: handleResponse)
    }

    func addCertificateDirectlyWithBpt(url: URL, secret: String, secretSignature: String, clientId: String?, clientIdSignature: String?, bpt: String) {
        activeDirectLinkURL = url

        let request: URLRequest

        if let clientId = clientId, let clientIdSignature = clientIdSignature {
            request = getRequestWithBpt(secret: secret, secretSignature: secretSignature, clientId: clientId, clientIdSignature: clientIdSignature, bpt: bpt)
        } else {
            request = getRequestWithBpt(secret: secret, secretSignature: secretSignature, bpt: bpt)
        }
    ...
    func getRequestWithBirthday(secret: String, secretSignature: String, clientId: String, clientIdSignature: String, birthday: Date?) -> URLRequest {
        let components = NSCalendar.current.dateComponents([.day, .month, .year], from: (birthday ?? Date()) as Date)
        let json: [String: Any] = [
            "secret": secret,
            "secretSignature": secretSignature,
    ...
    func getRequestWithBpt(secret: String, secretSignature: String, clientId: String, clientIdSignature: String, bpt: String) -> URLRequest {
        let json: [String: Any] = [
            "secret": secret,
            "secretSignature": secretSignature,
            "clientId": clientId,
    ...
    func addCertificate(qrCode: String) {
        ...
        case .success:
            ...
            activeDirectLinkURL = nil
        case .failure:
            onError()
        }
    }

    private func showErrorAboutMissingQRCode(url _: URL) {
